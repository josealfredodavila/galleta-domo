...
function updateUserTokens(wallet, tokens, callback) {
    db.run(
        `UPDATE users SET tokens = tokens + ? WHERE wallet_address = ?`,
        [tokens, wallet],
        function(err) {
            callback(!err);
        }
...
function canjearNft(wallet, callback) {
    db.get(
        `SELECT tokens, ha_canjeado FROM users WHERE wallet_address = ?`,
        [wallet],
        (err, user) => {
            if (err || !user) {
            ...
            db.run(
                `UPDATE users SET tokens = tokens - 12, ha_canjeado = 1 WHERE wallet_address = ?`,
                [wallet],
                function(err) {
                    if (err) {
