...
ChatMessageSchema.statics = {
    // Obtener mensajes de un stream con paginación
    ...
    // Guardar mensaje con manejo de errores
    async saveMessage(data) {
        try {
            const message = new this(data);
            return await message.save();
        } catch (error) {
            console.error('Error guardando mensaje de chat:', error.message);
            return null;
        }
    },

    // Eliminar mensajes antiguos (más de X días)
    async cleanOldMessages(daysToKeep = 30) {
        ...
        console.log(`🧹 Eliminados ${result.deletedCount} mensajes antiguos de chat`);
        return result;
    }
};
